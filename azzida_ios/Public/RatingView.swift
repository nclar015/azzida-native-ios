...
//  Numbster
//
//  Created by Vishnu Chhipa on 07/04/20.
...
open class RatingView: UIView {
    ...
    func customInit() {
        ...
        if offImage == nil {
            offImage = UIImage(named: "star", in: bundle, compatibleWith: self.traitCollection)
        }
        if onImage == nil {
            onImage = UIImage(named: "starFill", in: bundle, compatibleWith: self.traitCollection)
        }
        
        if halfImage == nil {
            halfImage = UIImage(named: "starFill", in: bundle, compatibleWith: self.traitCollection)
        }
    ...
    func layoutStars() {
        if stars.count != 0,
            ...
            var i = 1
            for iv in stars {
                if starSize == "tablecell"{
                    iv.frame = CGRect(x: CGFloat(i) * 16, y: 3, width: 15, height: 15)
                    //                        iv.center = CGPoint(x: CGFloat(i) * distance + halfWidth * CGFloat(i - 1),
                    //                                            y: self.frame.size.height/2 + 3)
                    i += 1
                }
                else{
                    iv.frame = CGRect(x: 0, y: 0, width: 25, height: 25)
